...
IF NOT EXISTS box
(id VARCHAR
(32) PRIMARY KEY,hostIp VARCHAR
(32),createTime VARCHAR
(32),version VARCHAR
(32),dateTime VARCHAR
(32));
-- 用户表 
CREATE TABLE
IF NOT EXISTS user(id VARCHAR
(32) PRIMARY KEY,openId VARCHAR
(32),createTime VARCHAR
(32),hostId VARCHAR
(32),FOREIGN KEY
(hostId) REFERENCES box
(id));


-- 盒子流水表 event:{1:check_update,2:set_update,3:pull_waiqin_data,4:report_ip} result:{1:check_udpate,2:set_update}
CREATE TABLE
IF NOT EXISTS box_water
(id INTEGER PRIMARY KEY AUTOINCREMENT,hostId VARCHAR
(32),hostIp VARCHAR
(32),version VARCHAR
(43),event VARCHAR
(32),result VARCHAR
...
CREATE TABLE
IF NOT EXISTS user_water
(id INTEGER PRIMARY KEY AUTOINCREMENT,userId VARCHAR
(32),openId VARCHAR
(32),hostId VARCHAR
(32),event VARCHAR
(32),result VARCHAR
...
UPDATE box
set id
='hostIdTestAbcd' where id ='testboxid00001';




CREATE TABLE
IF NOT EXISTS box_water_20170714
(id INTEGER PRIMARY KEY AUTOINCREMENT,hostId VARCHAR
(32),hostIp VARCHAR
(32),version VARCHAR
(43),event VARCHAR
...
CREATE TABLE
IF NOT EXISTS user_water_20170718
(id INTEGER PRIMARY KEY AUTOINCREMENT,userId VARCHAR
(32),openId VARCHAR
(32),hostId VARCHAR
(32),event VARCHAR
...
INSERT INTO user_authority
    (id,userId,warn)
VALUES
    (null, 'userIdTestSyys', 1);



select distinct hostId, box.company, box.dateTime
from user_water_20170717 as ua left join box on  ua.hostId=box.id
where hostId not in (select distinct hostId
from user_water_20170717
where event = 'push_request_login');



select *
from box
where id not in (select distinct hostId
from user_water_20170717
where event = 'push_request_login');
...
select *
from box
where id in (select distinct hostId
from user_water_2017
where)
...
SELECT *
from box
WHERE (SELECT count(DISTINCT userId), hostId
from user_water_20170717
where hostId='hostIdTestAbcd')>2;
...
from user_water_20170717
where hostId='hostIdTestAbcd';


SELECT *
from box
where id in (SELECT hostId
from user_water_20170717
GROUP BY hostId
...
SELECT box.*, tac.num
from box left join (SELECT count(DISTINCT userId) as num, hostId
    from user_water_20170717
    GROUP BY hostId) as tac on tac.hostId = bo
...
-- 有check_update 且版本号正常的盒子
SELECT distinct hostId
from box_water_20170717
where event = 'check_update' and version like '_%,%';


SELECT DISTINCT hostId
from user_water_20170717
where event = 'push_request_login' and hostId in (SELECT distinct hostId
    from box_water_20170717
    where event = 'check_update' and version like '_%,%');
...
-- select distinct hostId from user_water_${dayStr} where event = ?
...
-- 找出用户流水中没有 pull的盒子 
select hostId from box where bpx

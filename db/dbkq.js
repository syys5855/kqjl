...
exports.findExceptionBoxs = (dayStr, event) => {
    return new Promise((res, rej) => {
        db.all(`select * from box where recwarn = 'true' and id not in (select distinct hostId from user_water_${dayStr} where event = ?) and id in (SELECT distinct hostId from box_water_${dayStr} where event = 'check_update' and version like '_%,%');`, event, (err, data) => {
            err ? rej(err) : res(data);
        });
    });
}
